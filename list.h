...
static inline int
hlist_is_last (const struct hlist_node *list)
{
	return list->next == NULL;
}


//move hash list element to another head
static inline void
hlist_move (struct hlist_node *list, struct hlist_head* head)
{
	__hlist_del(list);
...
{
	//BUG_ON(to->first != NULL); //jeanfixme: check if move the BUG_ON to common include header
	struct hlist_node *first = from->first;
	to->first = first; //move all bx data to job

	if (first) //has entry
		first->pprev = &to->first;
	from->first = NULL;
}
